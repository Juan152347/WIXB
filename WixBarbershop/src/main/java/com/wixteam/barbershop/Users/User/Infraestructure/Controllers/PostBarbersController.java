...
public class PostBarbersController {
    ...
    @Operation(summary = "Create a new Barber", description = "Create a new Barber in the system", tags = {"Users"})
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Barber created"),
            @ApiResponse(responseCode = "400", description = "Invalid input", content = @Content(schema = @Schema(implementation = ErrorSchema.class))),
            @ApiResponse(responseCode = "409", description = "Barber already exist", content = @Content(schema = @Schema(implementation = ErrorSchema.class)))
    })

    @PostMapping(value = "/Barber/Create/")
    public ResponseEntity execute(@RequestBody BarbarRequest request ){
