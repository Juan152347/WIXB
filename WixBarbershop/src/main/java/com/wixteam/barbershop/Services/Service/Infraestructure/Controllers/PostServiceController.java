...
public class PostServiceController {
    ...
    @Operation(summary = "Create a new Services", description = "Create a new Services in the system", tags = {"Services"})
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Services created"),
            @ApiResponse(responseCode = "400", description = "Invalid input", content = @Content(schema = @Schema(implementation = ErrorSchema.class))),
            @ApiResponse(responseCode = "409", description = "Services already exist", content = @Content(schema = @Schema(implementation = ErrorSchema.class)))
    })

    @PostMapping(value = "/Service/Create")
    public ResponseEntity execute3(@RequestBody ServiceRequest request ){
        create.execute( request.getServiceId(), request.getServicePrice(), request.getServiceDetails());
        return ResponseEntity.status(HttpStatus.CREATED).body(null);
    }

    @ExceptionHandler(value = {InvalidCustomUUID.class, InvalidPriceService.class, CreateServiceFailed.class})//Se realiza para todos los controladores
